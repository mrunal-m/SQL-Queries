...
AND date(usage_date) = DATE_SUB(@run_date, INTERVAL 2 DAY)
--usage_date = current_date() - 2
--GROUP BY 1, 2, 3, 4, 5
),
...
WHERE date(t1.usage_date) = DATE_SUB(@run_date, INTERVAL 2 DAY)
AND date(t2.day) = DATE_SUB(@run_date, INTERVAL 2 DAY)
order by 6 desc 
)
...
from sc-bigquery-product-tools.penny_gauge_multi_region_us.non_cdn_view
WHERE date(usage_date) = DATE_SUB(@run_date, INTERVAL 2 DAY)
