SELECT DISTINCT tt1.*, tt2.team_name, tt2.pod_name,
...
from (WITH t1 AS (SELECT distinct date(creation_time, "Asia/Kolkata") dt, project_id, user_email, job_id, total_slot_ms,
...
t4 AS (SELECT distinct t2.owner_email_id,t2.scheduled_query_name, t2.refresh_schedule, t3.transferConfigID, t3.BQJobID from t2 INNER JOIN t3 ON
( t2.transfer_config_id) = ( t3.transferConfigID))

SELECT DISTINCT t1.dt, t1.project_id, t1.user_email, t4.scheduled_query_name, t4.refresh_schedule, t4.transferConfigID,  --job_id, runtime, total_slot_ms
...
group by all) tt1
LEFT JOIN maximal-furnace-783.data_platform_temp1.bq_compute_service_pod_mapping tt2
ON tt1.user_email = tt2.user_email
order by 1 ASC
