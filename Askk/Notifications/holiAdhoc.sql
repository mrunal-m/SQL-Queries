...
WITH users AS (SELECT DISTINCT CURRENT_DATE("Asia/Kolkata") date, 8 hour, 
SAFE_CAST(distinct_id AS STRING) userId, '' userName, '' phoneNo, 'adhocHoli' state
...
FROm users INNER JOIN templates ON templates.state = 'adhocHoli'
),
...
SELECT SAFE_CAST(date AS TIMESTAMP) date, hour, userId, userName, phoneNo, target, '' category, 'adhocHoli' cohort,
