...
SAFE_CAST(distinct_id AS STRING) userId, '' userName, '' phoneNo, 'Christmas' state
...
FROm users INNER JOIN templates ON templates.state = 'Christmas'
),
...
SELECT SAFE_CAST(date AS TIMESTAMP) date, hour, userId, userName, phoneNo, target, '' category, 'Adhoc_Christmas' cohort,
