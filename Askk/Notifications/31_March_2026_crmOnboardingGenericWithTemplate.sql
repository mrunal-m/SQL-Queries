...
copy_cycle AS (
SELECT DISTINCT userId, 'askk_onboarding' type, templateId 
FROM `maximal-furnace-783.askk_analytics.crmOnboardingGenericWithTemplate`
WHERE DATE(date, "Asia/Kolkata") >= CURRENT_DATE("Asia/Kolkata") - 3
UNION DISTINCT
SELECT DISTINCT SAFE_CAST(distinct_id AS STRING) userId, type,
...
matchedAll AS (
SELECT users.* except(state), templates.*,
FROm users INNER JOIN templates ON templates.state = 'onboardingGeneric'
),
...
SELECT SAFE_CAST(date AS TIMESTAMP) date, hour, userId, userName, phoneNo, target, '' category, state as cohort,
