...
WITH users AS (SELECT DISTINCT CURRENT_DATE("Asia/Kolkata") date, 10 hour, 
SAFE_CAST(distinct_id AS STRING) userId, '' userName, '' phoneNo, 'New_Year' state
...
FROm users INNER JOIN templates ON templates.state = 'New_Year'
),
...
SELECT SAFE_CAST(date AS TIMESTAMP) date, hour, userId, userName, phoneNo, target, '' category, 'Adhoc_New_Year' cohort,
