...
rechargeClicked AS (
SELECT DISTINCT callId, JSON_VALUE(meta, '$.payRequestId') payRequestId FROM  v2_base
WHERE mediaType = 'audio' AND action = 'rechargeClicked'
AND LENGTH(JSON_VALUE(meta, '$.avSwitchEligibleId'))>1),

rechargeSuccess AS (
SELECT DISTINCT callId, JSON_VALUE(meta, '$.payRequestId') payRequestId
FROM v2_base
WHERE mediaType = 'audio' AND action = 'paymentSuccessNudge' --AND LENGTH(JSON_VALUE(meta, '$.avSwitchEligibleId'))>1
),
...
COUNT(DISTINCT rechargePopup.callId) rechargePopup,
COUNT(DISTINCT rechargeClicked.callId) rechargeClicked,
COUNT(DISTINCT rechargeSuccess.callId) rechargeSuccess,
...
LEFT JOIN rechargePopup ON eligibleReqSuccess.callId = rechargePopup.callId
LEFT JOIN rechargeClicked ON rechargePopup.callId = rechargeClicked.callId
LEFT JOIN rechargeSuccess ON rechargeClicked.callId = rechargeSuccess.callId AND rechargeClicked.payRequestId = rechargeSuccess.payRequestId
LEFT JOIN videoReqMade ON rechargeSuccess.callId = videoReqMade.callId
