...
base as(select distinct a.title, a.text, if(a.state like '%call%','call',a.state) state, a.templateId,  
...
order by 1, 2),
base2 AS (
select distinct f.title, f.text, base.*except(title, text, initiated, issued, clicked), SUM(initiated) initiated, SUM(issued) issued, SUM(clicked) clicked  from base INNER join f on base.templateId= f.templateId
GROUP BY ALL
UNION ALL
SELECT base.*except(initiated, issued, clicked), SUM(initiated) initiated, SUM(issued) issued, SUM(clicked) clicked FROM base WHERE LOWER(templateId) LIKE "%adhoc%"
GROUP BY ALL)
