create temp function start_date() as (DATE_SUB(current_date('Asia/Kolkata'), INTERVAL 30 DAY));
...
with sent as (
  ...
  UNION ALL
  (SELECT date(date), 
  'fz_gold_match' cohort, 
  count(distinct userId) as users 
  FROM `maximal-furnace-783.vibely_analytics.crmNotificationsGoldHostWithTemplate`
  where date(date) between start_date() and end_date()
  group by 1, 2)
),
...
a as( 
  select distinct date(time,'Asia/Kolkata') as date, LOWER(clientType) clientType, communityNotifId,
  case when SPLIT(communityNotifId, '/')[0]='fz_realtime' and SPLIT(communityNotifId, '/')[SAFE_OFFSET(4)] ='VIDEO' then 'realtimeVideo'
  when SPLIT(communityNotifId, '/')[0]='fz_realtime' and SPLIT(communityNotifId, '/')[SAFE_OFFSET(4)] ='AUDIO' then 'realtime'
  ...
  WHEN LOWER(communityNotifId) LIKE "%fz_gold_match%" then 'fz_gold_match'
  else SPLIT(communityNotifId, '/')[SAFE_OFFSET(1)] end as state,
  SPLIT(communityNotifId, '/')[SAFE_OFFSET(2)] target,
...
b as(
  ...
  SPLIT(communityNotifId, '/')[SAFE_OFFSET(2)] as target,
  if(length(SPLIT(communityNotifId, '/')[SAFE_OFFSET(4)])<36, SPLIT(communityNotifId, '/')[SAFE_OFFSET(6)],SPLIT(communityNotifId, '/')[SAFE_OFFSET(4)]) id,
  distinct_id userID
...
ho as (
  SELECT distinct date(time,'Asia/Kolkata')date, cast(distinct_id as string) userid, referrer,
ARRAY_TO_STRING( (SELECT ARRAY_AGG(part ORDER BY offset)
FROM UNNEST(SPLIT(referrer,'_')) part WITH OFFSET
       WHERE offset BETWEEN 2 AND ARRAY_LENGTH(SPLIT(referrer,'_')) - 2),
      "_"
    ) AS new_cohort,
SPLIT(referrer,'_')[OFFSET(ARRAY_LENGTH(SPLIT(referrer,'_')) - 1)] AS id, sessionId 
...
feed as (
  ...
  split(userid,'U')[SAFE_OFFSET(1)] userid, 
  sessionId
...
qdau as (
  ...
  split(userId,'U')[SAFE_OFFSET(1)] userId, sessionId 
  ...
  union all
  SELECT distinct date(time,'Asia/Kolkata') date,split(userId,'U')[SAFE_OFFSET(1)] userId,sessionID
  ...
  select
  distinct 
  date(time,'Asia/Kolkata') date,split(userId,'U')[SAFE_OFFSET(1)] userId,sessionID 
...
with a as(
  ...
  SPLIT(communityNotifId, '/')[SAFE_OFFSET(2)] target,
  SPLIT(communityNotifId, '/')[SAFE_OFFSET(3)] copy,
  if(length(SPLIT(communityNotifId, '/')[SAFE_OFFSET(4)])<36, SPLIT(communityNotifId, '/')[SAFE_OFFSET(6)],SPLIT(communityNotifId, '/')[SAFE_OFFSET(4)]) id,
  distinct_id userID from `maximal-furnace-783.vibely_analytics.notification_initiated`
