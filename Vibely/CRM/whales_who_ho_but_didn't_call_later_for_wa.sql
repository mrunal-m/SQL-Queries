...
AND timestamp(time) BETWEEN TIMESTAMP('2025-09-24 07:30:00 UTC') AND TIMESTAMP('2025-09-24 10:30:00 UTC')
AND tenant = 'fz'
),
...
complete_call AS (SELECT consultee_user_id  userId from `maximal-furnace-783.sc_analytics.consultation` a 
where date(time, "Asia/Kolkata") >= "2024-10-01"
and consultation_type = 'FIND_A_FRIEND'
and tenant = "fz" AND status = 'complete'
AND timestamp(time) BETWEEN TIMESTAMP('2025-09-24 10:00:00 UTC') AND TIMESTAMP('2025-09-24 16:00:00 UTC')),
