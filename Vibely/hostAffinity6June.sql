...
select date, hour, type, userId, hostId, hostUserId, 
...
where 
((rnk<=5 and _30DaysCalls>=1) or (_7DaysGMV>=100 and rnk<=5) or (_303minCalls>=1 and rnk<=5) ) OR 
((LOWER(affinity)='video' and rnk<=25) or (LOWER(affinity)= 'video' AND _30DaysVideoGMV >=350))
order by date, userid, rnk)
order by date, hostid, rnk
