CREATE OR REPLACE TABLE `maximal-furnace-783.vibely_analytics.crmVibelyAdhocRetool_1Jan2026_gifters`
AS 
(
with a as (
...
'Exciting ways to wish.. 💖' title, 'a Happy New Year 2026! ✨' text
...
where date(time,'Asia/Kolkata') >= '2025-10-01' AND source_currency = "COIN"
           ...
           AND entity in ("FZ_CONSULTATION_GIFTING",
                          'FZ_CONSULTATION',
                          'VIDEO_CONSULTATION',
                          'VIDEO_CONSULTATION_GIFTING')
)
),
...
'feed' as target,
'adHoc' cohort,
'AdhocRetool_1Jan2026_gifters' as templateId,
a.title, 
a.text, 
a.thumbnailUrl, 
notifRank, 
