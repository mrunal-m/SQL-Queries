...
-- rnk, batchsize, delay introduced
select affinity, count(*) from (
select DISTINCT date, hour, type, userId, hostId, hostUserId, 
ROW_NUMBER() OVER(PARTITION BY hostUserId ORDER by 0.70*(_7DaysGMV)+0.15*(_30DaysGMV)+0.15*(_3DaysGMV) DESC) rnk, 
avgPCT, _3DaysGMV, _7DaysGMV, _30DaysGMV, userName, _3DaysVideoGMV, _7DaysVideoGMV, _30DaysVideoGMV, affinity 
from (
...
select *except( _30DaysCalls,_303minCalls) from (
...
when _30DaysCalls>=1 then 'call' end as type,
 gmv.userId, gmv.hostId, hostUserId, 
 rnk, avgPCT,
 _3DaysGMV,
 _7DaysGMV,
 _30DaysGMV,
...
where 
((rnk<=6 and _30DaysCalls>=1) or (_7DaysGMV>=100 and rnk<=5) or (_303minCalls>=1 and rnk<=5) ) OR 
((LOWER(affinity)='video' and rnk<=25) or (LOWER(affinity)= 'video' AND _30DaysVideoGMV >=300))
order by date, userid, rnk)
order by date, hostid, rnk) group by 1
