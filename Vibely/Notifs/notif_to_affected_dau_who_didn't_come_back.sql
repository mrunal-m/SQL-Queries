CREATE OR REPLACE TABLE `maximal-furnace-783.vibely_analytics.crmVibelyAdhocRetool_FeedIssue_24Sep2`
AS (
with a as (
WITH main as (
WITH a as (
...
a1 AS (SELECT distinct SAFE_CAST(a.distinct_id AS STRING) userId, b.phoneNo, b.name userName, b.language,
from `maximal-furnace-783.vibely_analytics.home_opened` a
INNER JOIN `maximal-furnace-783.sc_analytics.user`b ON
a.distinct_id = SAFE_CAST(b.id AS INT64)
WHERE date(a.time, "Asia/Kolkata") >= "2025-09-24" AND 
(TIMESTAMP(a.time) >=('2025-09-24 12:00:00 UTC')) AND b.tenant = 'fz'
 )
SELECT a.* FROM a LEFT JOIN a1 ON a.userId = a1.userId
WHERE a1.userId IS NULL
)

SELECT main.*,  'https://cdn-sc-g.sharechat.com/33d5318_1c8/tools/1fb57083_1729256150911_sc.png' thumbnailUrl, 
'Tried to call? We are back! 🚀' title, 'Call now 💌 continue talking!' text FROM main
),
