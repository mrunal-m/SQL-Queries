...
with
usr AS (
  SELECT distinct consultant_id chatroomId,consultation_language language
...
and consultation_type = 'FIND_A_FRIEND' and tenant='fz'
)
) a
where row_num = 1),
consultation as (
select *,
TIMESTAMP_DIFF((CAST(session_ended_at AS TIMESTAMP)), (CAST(session_started_at AS TIMESTAMP)), SECOND) AS conversationDuration from (
...
lastType as(
select distinct consultee_user_id userId,consultant_id hostId,media_type, row_number() over (partition by consultee_user_id,consultant_id order by time desc)lRn from consultation
where
status = 'completed'
and total_charges_new > 0
qualify lRn=1
),

pairs as (
select distinct DATE(time,'Asia/Kolkata')date, consultee_user_id as userID,language,consultation_id,consultant_id hostID,
consultant_user_id hostUserId,
total_charges_new,media_type,SUM (CAST(ConversationDuration/60 AS FLOAT64)) AS paid_call_time,
FROM consultation
where
status = 'completed'
and total_charges_new > 0
...
pct as(
  select  hostID, sum(paid_call_time)/count(distinct consultation_id) avgPCT from pairs
  where date>=current_date('Asia/Kolkata')-7
  group by 1
),

gmv as(
    select *, row_number() over (partition by date, userid order by _3DayRank+_7DayRank+_30DayRank)rnk from 
  (
...
from (
  select date, userId, hostId,any_value(hostUserId)hostUserId, sum(total_charges_new) GMV, count(distinct consultation_id) calls,
...
select *except( _30DaysCalls,
 _303minCalls) 
 
 from (select distinct current_date('Asia/Kolkata')date,extract(hour from current_datetime('Asia/Kolkata'))hour, case when 
(_7DaysGMV>=100 ) then 'GMV'
when _303minCalls>=1 then '3minCall' 
when _30DaysCalls>=1 then 'call' end as type,
 gmv.userId, gmv.hostId,hostUserId,rnk, avgPCT,
 _3DaysGMV,
 _7DaysGMV,
 _30DaysGMV,name userName,
 _3DaysVideoGMV,
 _7DaysVideoGMV,
 _30DaysVideoGMV,
 case 
 when _3DaysGMV!=0 and _3DaysVideoGMV>=_3DaysGMV - _3DaysVideoGMV then 'VIDEO' 
 when _7DaysGMV!=0 and _7DaysVideoGMV>=_7DaysGMV - _7DaysVideoGMV then 'VIDEO' 
 when _30DaysGMV!=0 and _30DaysVideoGMV>=_30DaysGMV - _30DaysVideoGMV then 'VIDEO' 
...
where  ((rnk<=3 and _30DaysCalls>=1) or (_7DaysGMV>=100 and rnk<=5) or (_303minCalls>=1 and rnk<=5) ) or(
affinity='video' and rnk<=20)
