...
base as(select date, variant, 
if(state like '%call%','call',state)state, 
...
select a.date, exp_base.variant, a.state, a.clientType, a.id aid, b.id as bid, c.id as cid,a.userid as auserid,b.userid as buserid, c.userid as cuserid, ho.userid as hoUserid,f.userid as fUserId,q.userid as qUserid, ap.user_id as apUserId, ap.total_charges, ap.call_time,
wc.user_id as wc_uid,
row_number() over (partition by a.date,a.userId,ap.call_id order by ap.time)rn
from exp_base LEFT JOIN a On exp_base.userId = SAFE_CAST(a.userId AS STRING)
LEFT join b on 
